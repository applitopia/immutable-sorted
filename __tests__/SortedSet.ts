...
import { is, List, Map, SortedMap, OrderedSet, SortedSet, Seq, Set } from '../';
...
describe('SortedSet', () => {
  ...
  it('accepts array-like of values', () => {
    let s = SortedSet<any>({ length: 3, 1: 2 } as any);
  ...
  it('unions two sets', () => {
    let s1 = SortedSet.of('a', 'b', 'c');
    let s2 = SortedSet.of('d', 'b', 'wow');
    let s3 = s1.union(s2);
  ...
  it('unions a set and another collection and returns a set', () => {
    ...
    let emptySortedSet = SortedSet();
    let l = List([1, 2, 3]);
    let s2 = s1.union(l);
    let s3 = emptySortedSet.union(l);
    let o = OrderedSet([1, 2, 3]);
    let s4 = s1.union(o);
    let s5 = emptySortedSet.union(o);
  ...
  it('deletes down to empty set', () => {
    let s = SortedSet.of('A').remove('A');
    expect(s).toEqual(SortedSet());
  });

  it('unions multiple sets', () => {
    let s = SortedSet.of('A', 'B', 'C').union(SortedSet.of('C', 'D', 'E'), SortedSet.of('D', 'B', 'F'));
    expect(s).is(SortedSet.of('A', 'B', 'C', 'D', 'E', 'F'));
  });

  it('intersects multiple sets', () => {
    let s = SortedSet.of('A', 'B', 'C').intersect(SortedSet.of('B', 'C', 'D'), SortedSet.of('A', 'C', 'E'));
    expect(s).is(SortedSet.of('C'));
  });

  it('diffs multiple sets', () => {
    let s = SortedSet.of('A', 'B', 'C').subtract(SortedSet.of('C', 'D', 'E'), SortedSet.of('D', 'B', 'F'));
  ...
  describe('accepts Symbol as entry #579', () => {
    ...
    it('operates on small number of symbols, preserving set uniqueness', () => {
      ...
      let symbolSet = SortedSet([ a, b, c, a, b, c, a, b, c, a, b, c ]);
    ...
    it('operates on a large number of symbols, maintaining obj uniqueness', () => {
      ...
      let symbolSet = SortedSet(manySymbols);
  ...
  it('can use intersect after add or union in a withMutation', () => {
    let set = SortedSet(['a', 'd']).withMutations(s => {
      s.add('b');
      s.union(['c']);
      s.intersect(['b', 'c', 'd']);
    });
